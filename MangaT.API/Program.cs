...
using MangaT.API.Logging;
using MangaT.API.OpenApi;
using MangaT.API.Security;
...
// Logger de arranque: captura errores antes de cargar appsettings/Serilog completo.
SerilogExtensions.ConfigureBootstrapLogger();

try
{
    Log.Information("Iniciando MangaT API");

    var builder = WebApplication.CreateBuilder(args);

    // Serilog reemplaza ILogger<T> en toda la aplicación (config en appsettings.json).
    builder.AddMangaTSerilog();
    ...
    {
        // Política nombrada: alternativa legible a [Authorize(Roles = "Admin")].
        // Liskov: cualquier usuario Admin satisface la misma regla de autorización.
        options.AddPolicy("AdminOnly", policy => policy.RequireRole("Admin"));
    });

    // Dependency Inversion: ApplicationCore usa ICurrentUserContext; la API lo implementa con HttpContext.
    ...
    if (builder.Environment.IsEnvironment("Testing"))
    {
        ...
        builder.Services.AddInfrastructureAuth();
    ...
    // Una línea por petición HTTP con duración, status y TraceId.
    app.UseMangaTRequestLogging();
    ...
    Log.Information("MangaT API lista en {Environment}", app.Environment.EnvironmentName);
    app.Run();
}
catch (Exception ex)
{
    Log.Fatal(ex, "MangaT API terminó inesperadamente");
    throw;
}
finally
{
...
static async Task MigrateDatabaseWithRetryAsync(IServiceProvider services)
{
    ...
    for (var attempt = 1; attempt <= maxRetries; attempt++)
    {
        ...
        catch (Exception ex) when (attempt < maxRetries)
        {
            Log.Warning(
                ex,
                "Migración fallida (intento {Attempt}/{MaxRetries}). Reintentando en 3 s…",
                attempt,
                maxRetries);
    ...
    Log.Fatal("No se pudieron aplicar las migraciones tras {MaxRetries} intentos", maxRetries);
    throw new InvalidOperationException("No se pudo conectar a la base de datos para aplicar migraciones.");
}
